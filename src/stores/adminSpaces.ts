...
export const Spaces = defineStore('Spaces', {
  ...
  actions: {
      ...
      .then(response => 
        {
          ...
          const selectedRoomType = response.data.roomTypes.find(roomType => roomType.spaceTypeId === response.data.roomTypeId)
          if (selectedRoomType) {
            this.roomTypeSelected = {
              id: formattedList.findIndex(option => option.aditionalInfo === selectedRoomType.spaceTypeId),
    ...
    async saveSpace() {
      loadingService.show('Loading...');
      adminAPI.post('/Space/' + cookies.get('spaceId'),
      {
      ...
      .then(() => 
        {
          loadingService.close();
          toastService.show('Success', 'Space details updated', 'success', 'top');
        }
      ).catch(error =>{
        toastService.show('Error', error, 'error', 'top');
      })
    ...
    async saveSpaceDocument() {
      ...
      .then(() => 
        {
          loadingService.close();
          toastService.show('Success', 'Document uploaded successfully', 'success', 'top');
        }
      ).catch(error =>{
        toastService.show('Error', error, 'error', 'top');
      })
      ...
      .then(() => 
        {
          ...
          toastService.show('Success', 'Document removed successfully', 'success', 'top');
        }
      ).catch(error =>{
        toastService.show('Error', error, 'error', 'top');
      })
    ...
    async editSpacesDevices(deviceIndex: number) {
      ...
      .then(() => 
        {
          loadingService.close();
          toastService.show('Success', 'Space devices updated', 'success', 'top');
        }
      ).catch(error =>{
        toastService.show('Error', error, 'error', 'top');
      })
    ...
    async editSpacesWifi() {
      ...
      '&WifiPassword=' + this.wifi.wifiPassword
      ).then(() => 
        {
          this.getSpaceDetailsDevices()
          loadingService.close()
          toastService.show('Success', 'Space wifi details updated', 'success', 'top')
        }
      ).catch(error =>{
        toastService.show('Error', error, 'error', 'top');
      })

...
(function () {
  ...
  let Inherit = function (ce, ce2) {
    ...
    if (typeof Object.getOwnPropertyNames === "undefined") {
      ...
      let props = Object.getOwnPropertyNames(ce2.prototype);
  ...
  ClipperLib.PolyTree.prototype.Clear = function () {
    for (let i = 0, ilen = this.m_AllPolys.length; i < ilen; i++) this.m_AllPolys[i] = null;
  ...
  /*
   ClipperLib.FPoint.prototype.Equals = function (obj)
   {
     ...
     {
         let  a = Cast(obj, ClipperLib.FPoint);
  ...
  ClipperLib.ClipperBase.prototype.ProcessBound = function (E, LeftBoundIsForward) {
    ...
    if (Result.OutIdx === ClipperLib.ClipperBase.Skip) {
      ...
      if (E === Result) {
        ...
        let locMin = new ClipperLib.LocalMinima();
        locMin.Next = null;
        locMin.Y = E.Bot.Y;
  ...
  ClipperLib.ClipperBase.prototype.AddPaths = function (ppg, polyType, closed) {
    let result = false;
    for (let i = 0, ilen = ppg.length; i < ilen; ++i) if (this.AddPath(ppg[i], polyType, closed)) result = true;
    return result;
  };
  ...
  ClipperLib.ClipperBase.prototype.InsertScanbeam = function (Y) {
    ...
    if (this.m_Scanbeam === null) {
      ...
      let newSb = new ClipperLib.Scanbeam();
      newSb.Y = Y;
      ...
      let newSb1 = new ClipperLib.Scanbeam();
      newSb1.Y = Y;
  ...
  ClipperLib.ClipperBase.prototype.CreateOutRec = function () {
    let result = new ClipperLib.OutRec();
  ...
  ClipperLib.ClipperBase.prototype.SwapPositionsInAEL = function (edge1, edge2) {
    ...
    if (edge1.NextInAEL === edge2) {
      ...
      let next1 = edge1.NextInAEL;
      if (next1 !== null) {
        next1.PrevInAEL = edge2;
      }
      let prev1 = edge2.PrevInAEL;
      if (prev1 !== null) {
      ...
      let next2 = edge1.NextInAEL;
      let prev2 = edge1.PrevInAEL;
  ...
  ClipperLib.Clipper.prototype.InsertMaxima = function (X) {
    ...
    let newMax = new ClipperLib.Maxima();
    newMax.X = X;
  ...
  ClipperLib.Clipper.prototype.SetWindingCount = function (edge) {
    ...
    if (e === null) {
      let pft = edge.PolyTyp === ClipperLib.PolyType.ptSubject ? this.m_SubjFillType : this.m_ClipFillType;
      if (edge.WindDelta === 0) {
  ...
  ClipperLib.Clipper.prototype.AddLocalMinPoly = function (e1, e2, pt) {
    ...
    if (prevE !== null && prevE.OutIdx >= 0 && prevE.Top.Y < pt.Y && e.Top.Y < pt.Y) {
      let xPrev = ClipperLib.Clipper.TopX(prevE, pt.Y);
      let xE = ClipperLib.Clipper.TopX(e, pt.Y);
      if (
        xPrev === xE &&
        e.WindDelta !== 0 &&
        prevE.WindDelta !== 0 &&
        ClipperLib.ClipperBase.SlopesEqual5(new ClipperLib.FPoint2(xPrev, pt.Y), prevE.Top, new ClipperLib.FPoint2(xE, pt.Y), e.Top)
      ) {
  ...
  ClipperLib.Clipper.prototype.AddOutPt = function (e, pt) {
    if (e.OutIdx < 0) {
      ...
      let newOp = new ClipperLib.OutPt();
      outRec.Pts = newOp;
      ...
      let op = outRec.Pts;
      let ToFront = e.Side === ClipperLib.EdgeSide.esLeft;
  ...
  ClipperLib.Clipper.prototype.SwapPoints = function (pt1, pt2) {
    let tmp = new ClipperLib.FPoint1(pt1.Value);
    //pt1.Value = pt2.Value;
  ...
  ClipperLib.Clipper.prototype.ProcessHorizontals = function () {
    let horzEdge = {}; //m_SortedEdges;
    while (this.PopEdgeFromSEL(horzEdge)) {
      this.ProcessHorizontal(horzEdge.v);
    }
  };

  ClipperLib.Clipper.prototype.GetHorzDirection = function (HorzEdge, $let) {
    if (HorzEdge.Bot.X < HorzEdge.Top.X) {
      $let.Left = HorzEdge.Bot.X;
      $let.Right = HorzEdge.Top.X;
      $let.Dir = ClipperLib.Direction.dLeftToRight;
    } else {
      $let.Left = HorzEdge.Top.X;
      $let.Right = HorzEdge.Bot.X;
      $let.Dir = ClipperLib.Direction.dRightToLeft;
    }
  };
  ...
  ClipperLib.Clipper.prototype.ProcessHorizontal = function (horzEdge) {
    ...
    this.GetHorzDirection(horzEdge, $let);
    let dir = $let.Dir;
    let horzLeft = $let.Left;
    let horzRight = $let.Right;
    ...
    for (;;) //loop through consec. horizontal edges
    {
      ...
      while (e !== null) {
        ...
        if (dir === ClipperLib.Direction.dLeftToRight) {
          let Pt = new ClipperLib.FPoint2(e.Curr.X, horzEdge.Curr.Y);
          this.IntersectEdges(horzEdge, e, Pt);
        } else {
          let Pt = new ClipperLib.FPoint2(e.Curr.X, horzEdge.Curr.Y);
          this.IntersectEdges(e, horzEdge, Pt);
        }
        let eNext = this.GetNextInAEL(e, dir);
  ...
  ClipperLib.Clipper.prototype.GetMaximaPairEx = function (e) {
    ...
    let result = this.GetMaximaPair(e);
    if (
      result === null ||
  ...
  ClipperLib.Clipper.prototype.ProcessEdgesAtTopOfScanbeam = function (topY) {
    ...
    while (e !== null) {
      ...
      if (IsMaximaEdge) {
        let eMaxPair = this.GetMaximaPairEx(e);
      ...
      if (IsMaximaEdge) {
        ...
        if (this.StrictlySimple) {
          ...
          if (
            ...
            let ip = new ClipperLib.FPoint1(e.Curr);
  ...
  ClipperLib.Clipper.prototype.BuildResult2 = function (polytree) {
    ...
    for (let i = 0, ilen = this.m_PolyOuts.length; i < ilen; i++) {
      ...
      let pn = new ClipperLib.PolyNode();
  ...
  ClipperLib.Clipper.prototype.DupOutPt = function (outPt, InsertAfter) {
    let result = new ClipperLib.OutPt();
    //result.Pt = outPt.Pt;
  ...
  ClipperLib.Clipper.prototype.JoinHorz = function (op1, op1b, op2, op2b, Pt, DiscardLeft) {
    let Dir1 = op1.Pt.X > op1b.Pt.X ? ClipperLib.Direction.dRightToLeft : ClipperLib.Direction.dLeftToRight;
    let Dir2 = op2.Pt.X > op2b.Pt.X ? ClipperLib.Direction.dRightToLeft : ClipperLib.Direction.dLeftToRight;
  ...
  ClipperLib.Clipper.prototype.JoinPoints = function (j, outRec1, outRec2) {
    ...
    if (isHorizontal && ClipperLib.FPoint.op_Equality(j.OffPt, j.OutPt1.Pt) && ClipperLib.FPoint.op_Equality(j.OffPt, j.OutPt2.Pt)) {
      ...
      let Reverse1 = op1b.Pt.Y > op1.Pt.Y || !ClipperLib.ClipperBase.SlopesEqual4(op1.Pt, op1b.Pt, j.OffPt);
      if (Reverse1) {
        op1b = op1.Prev;
      ...
      let Reverse2 = op2b.Pt.Y > op2.Pt.Y || !ClipperLib.ClipperBase.SlopesEqual4(op2.Pt, op2b.Pt, j.OffPt);
      if (Reverse2) {
        op2b = op2.Prev;
  ...
  ClipperLib.Clipper.GetBounds = function (paths) {
    ...
    let result = new ClipperLib.FRect();
  ...
  ClipperLib.Clipper.prototype.Poly2ContainsPoly1 = function (outPt1, outPt2) {
    ...
    do {
      ...
      let res = this.PointInPolygon(op.Pt, outPt2);
  ...
  ClipperLib.Clipper.SimplifyPolygon = function (poly, fillType) {
    ...
    let c = new ClipperLib.Clipper(0);
  ...
  ClipperLib.Clipper.CleanPolygon = function (path, distance) {
    ...
    for (let i = 0; i < cnt; ++i) outPts[i] = new ClipperLib.OutPt();
    for (let i = 0; i < cnt; ++i) {
      outPts[i].Pt = path[i];
  ...
  ClipperLib.Clipper.CleanPolygons = function (polys, distance) {
    let result = new Array(polys.length);
    for (let i = 0, ilen = polys.length; i < ilen; i++) result[i] = ClipperLib.Clipper.CleanPolygon(polys[i], distance);
    return result;
  };
  ...
  ClipperLib.Clipper.MinkowskiSum = function (pattern, path_or_paths, pathIsClosed) {
    if (!(path_or_paths[0] instanceof Array)) {
      let path = path_or_paths;
      let paths = ClipperLib.Clipper.Minkowski(pattern, path, true, pathIsClosed);
      let c = new ClipperLib.Clipper();
      c.AddPaths(paths, ClipperLib.PolyType.ptSubject, true);
      c.Execute(ClipperLib.ClipType.ctUnion, paths, ClipperLib.PolyFillType.pftNonZero, ClipperLib.PolyFillType.pftNonZero);
      return paths;
    } else {
      let paths = path_or_paths;
      let solution = new ClipperLib.Paths();
      let c = new ClipperLib.Clipper();
      for (let i = 0; i < paths.length; ++i) {
        let tmp = ClipperLib.Clipper.Minkowski(pattern, paths[i], true, pathIsClosed);
        c.AddPaths(tmp, ClipperLib.PolyType.ptSubject, true);
        if (pathIsClosed) {
          let path = ClipperLib.Clipper.TranslatePath(paths[i], pattern[0]);
  ...
  ClipperLib.Clipper.TranslatePath = function (path, delta) {
    let outPath = new ClipperLib.Path();
    for (let i = 0; i < path.length; i++) outPath.push(new ClipperLib.FPoint2(path[i].X + delta.X, path[i].Y + delta.Y));
    return outPath;
  };

  ClipperLib.Clipper.MinkowskiDiff = function (poly1, poly2) {
    let paths = ClipperLib.Clipper.Minkowski(poly1, poly2, false, true);
    let c = new ClipperLib.Clipper();
    c.AddPaths(paths, ClipperLib.PolyType.ptSubject, true);
    c.Execute(ClipperLib.ClipType.ctUnion, paths, ClipperLib.PolyFillType.pftNonZero, ClipperLib.PolyFillType.pftNonZero);
    return paths;
  ...
  ClipperLib.ClipperOffset.prototype.AddPath = function (path, joinType, endType) {
    ...
    if (highI < 0) return;
    let newNode = new ClipperLib.PolyNode();
    ...
    if (this.m_lowest.X < 0) this.m_lowest = new ClipperLib.FPoint2(this.m_polyNodes.ChildCount() - 1, k);
      ...
      let ip = this.m_polyNodes.Childs()[this.m_lowest.X].m_polygon[this.m_lowest.Y];
  ...
  ClipperLib.ClipperOffset.prototype.FixOrientations = function () {
    ...
    if (this.m_lowest.X >= 0 && !ClipperLib.Clipper.Orientation(this.m_polyNodes.Childs()[this.m_lowest.X].m_polygon)) {
      for (let i = 0; i < this.m_polyNodes.ChildCount(); i++) {
        let node = this.m_polyNodes.Childs()[i];
        if (
  ...
  ClipperLib.ClipperOffset.prototype.DoOffset = function (delta) {
    ...
    let steps = 3.14159265358979 / Math.acos(1 - y / Math.abs(delta));
    ...
    for (let i = 0; i < this.m_polyNodes.ChildCount(); i++) {
      ...
      if (node.m_endtype === ClipperLib.EndType.etClosedPolygon) {
        ...
        for (let j = len - 1; j > 0; j--) this.m_normals[j] = new ClipperLib.FPoint2(-this.m_normals[j - 1].X, -this.m_normals[j - 1].Y);
  ...
  ClipperLib.ClipperOffset.prototype.Execute = function () {
    ...
    if (!ispolytree) {
      ...
      let clpr = new ClipperLib.Clipper(0);
      clpr.AddPaths(this.m_destPolys, ClipperLib.PolyType.ptSubject, true);
      if (delta > 0) {
        clpr.Execute(ClipperLib.ClipType.ctUnion, solution, ClipperLib.PolyFillType.pftPositive, ClipperLib.PolyFillType.pftPositive);
      } else {
        let r = ClipperLib.Clipper.GetBounds(this.m_destPolys);
        let outer = new ClipperLib.Path();
      ...
      if (delta > 0) {
        ...
        if (solution.ChildCount() === 1 && solution.Childs()[0].ChildCount() > 0) {
          let outerNode = solution.Childs()[0];
 ...
 */ else if (this.m_sinA < -1) this.m_sinA = -1.0;
  ...
  ClipperLib.ClipperOffset.prototype.DoSquare = function (j, k) {
    let dx = Math.tan(Math.atan2(this.m_sinA, this.m_normals[k].X * this.m_normals[j].X + this.m_normals[k].Y * this.m_normals[j].Y) / 4);
    this.m_destPoly.push(
      new ClipperLib.FPoint2(
  ...
  ClipperLib.JS.BoundsOfPaths = function (paths) {
    let bounds = ClipperLib.Clipper.GetBounds(paths);
    return bounds;
  };

  // Clean() joins vertices that are too near each other
  // and causes distortion to offsetted polygons without cleaning
  ClipperLib.JS.Clean = function (_polygon, delta) {
    if (!(_polygon instanceof Array)) return [];
    let isPolygons = _polygon[0] instanceof Array;
    let polygon = ClipperLib.JS.Clone(polygon);
    ...
    let k_length = polygon.length;
    let len, poly, result, d, p, j, i;
    let results = [];
  ...
  ClipperLib.JS.Clone = function (polygon) {
    ...
    let isPolygons = polygon[0] instanceof Array;
    if (!isPolygons) polygon = [polygon];
  ...
  ClipperLib.JS.Lighten = function (polygon, tolerance) {
    ...
    let isPolygons = polygon[0] instanceof Array;
    if (!isPolygons) polygon = [polygon];
    let i, j, poly, k, poly2, plen, A, B, P, d, rem, addlast;
    let bxax, byay, l, ax, ay;
    let len = polygon.length;
  ...
  ClipperLib.JS.AddOuterPolyNodeToExPolygons = function (polynode, expolygons) {
    let ep = new ClipperLib.ExPolygon();
    ep.outer = polynode.Contour();
    let childs = polynode.Childs();
    let ilen = childs.length;
  ...
  ClipperLib.JS.PolyTreeToExPolygons = function (polytree) {
    let expolygons = new ClipperLib.ExPolygons();
